...
function setupALDUINAI(){
  ...
  if(window.matchMedia?.('(max-width:520px)').matches) aiEl('live2dAIDock')?.classList.add('collapsed');
...
function live2dParam(id,val){try{const core=live2dModel?.internalModel?.coreModel;if(!core)return false;const idx=core.getParameterIndex(id);if(idx<0)return false;let v=Number(val);if(!Number.isFinite(v))return false;const min=typeof core.getParameterMinimumValueByIndex==='function'?core.getParameterMinimumValueByIndex(idx):typeof core.getParameterMinimumValue==='function'?core.getParameterMinimumValue(id):-1;const max=typeof core.getParameterMaximumValueByIndex==='function'?core.getParameterMaximumValueByIndex(idx):typeof core.getParameterMaximumValue==='function'?core.getParameterMaximumValue(id):1;if(Number.isFinite(min)&&Number.isFinite(max))v=Math.max(min,Math.min(max,v));core.setParameterValueByIndex(idx,v);return true}catch{}return false}
const live2dEmotionState={name:'neutral',t:1};
function live2dApplyEmotionNow(){
  if(!live2dModel)return;
  const presets={
    neutral:{mouth:0,brow:0,eyeY:0,eyeOpen:1,mouthOpen:0},
    happy:{mouth:.78,brow:.16,eyeY:.08,eyeOpen:.92,mouthOpen:.04},
    sad:{mouth:-.55,brow:-.48,eyeY:-.10,eyeOpen:.78,mouthOpen:.02},
    angry:{mouth:-.38,brow:-.72,eyeY:-.04,eyeOpen:.88,mouthOpen:.02},
    surprised:{mouth:.12,brow:.52,eyeY:.22,eyeOpen:1,mouthOpen:.72}
  };
  const v=presets[live2dEmotionState.name]||presets.neutral;
  const speed=.22;
  const set=(key,id,target)=>{
    const stateKey='_'+key;
    const current=Number.isFinite(live2dEmotionState[stateKey])?live2dEmotionState[stateKey]:target;
    live2dEmotionState[stateKey]=current+(target-current)*speed;
    live2dParam(id,live2dEmotionState[stateKey]);
  };
  set('mouth','ParamMouthForm',v.mouth);
  set('browL','ParamBrowLY',v.brow); set('browR','ParamBrowRY',v.brow);
  set('eyeY','ParamEyeBallY',v.eyeY);
  set('eyeL','ParamEyeLOpen',v.eyeOpen); set('eyeR','ParamEyeROpen',v.eyeOpen);
  if(live2dEmotionState.name==='surprised')set('mouthOpen','ParamMouthOpenY',v.mouthOpen);
  else if(!aiEl('live2dTalk')?.checked)set('mouthOpen','ParamMouthOpenY',v.mouthOpen);
}
function live2dEmotion(emotion){
  if(!live2dModel)return;
  emotion=String(emotion||'neutral').toLowerCase();
  if(!['neutral','happy','sad','angry','surprised'].includes(emotion))emotion='neutral';
  live2dEmotionState.name=emotion;
  // If the model contains native .exp3 expressions, try them first. The
  // parameter preset below remains the universal fallback for models without expressions.
  try{
    const aliases={happy:['happy','joy','smile'],sad:['sad','cry','sorrow'],angry:['angry','mad'],surprised:['surprised','surprise','shock'],neutral:['neutral','normal']};
    for(const n of aliases[emotion]||[]){if(typeof live2dModel.expression==='function'){const r=live2dModel.expression(n);if(r!==false)break}}
  }catch{}
  live2dApplyEmotionNow();
...
async function loadLive2D(data){
  ...
  live2dApp.stage.addChild(live2dModel); live2dApp.ticker.add(live2dApplyEmotionNow); live2dCenter();
...
async function live2dLoadBundledFile(modelURL,name){
  try{
    live2dStatus('LOADING…');
    const ready=await live2dRuntime();
    if(!ready || !window.PIXI?.live2d?.Live2DModel) throw new Error('Live2D runtime не загрузился.');
    const response=await fetch(modelURL,{cache:'no-store'});
    if(!response.ok) throw new Error(name+' не найдена (HTTP '+response.status+').');
    const json=await response.json();
    if(!json?.FileReferences?.Moc) throw new Error('Некорректный model3.json у '+name+'.');
    if(live2dApp){try{live2dApp.destroy(true,{children:true,texture:true,baseTexture:true})}catch{}live2dApp=null;live2dModel=null}
    revokeLiveURLs();
    const canvas=aiEl('live2dCanvas'),host=aiEl('live2dStage');
    live2dApp=new PIXI.Application({view:canvas,autoStart:true,resizeTo:host,transparent:true,antialias:true,resolution:Math.min(2,window.devicePixelRatio||1)});
    ...
    live2dApp.ticker.add(live2dApplyEmotionNow);
    live2dCenter();
    live2dModel.on?.('hit',()=>{if(aiEl('live2dAI')?.checked){live2dEmotion('happy');live2dMotion('tap')}});
    aiEl('live2dEmpty').hidden=true;
    live2dStatus('MODEL ONLINE',true);
    aiEl('live2dModelInfo').textContent='✓ '+name+' • Cubism 3/4 • встроенная';
    live2dMotion('auto');
    save(LIVE2D_KEY,{name,source:'bundled',modelURL});
    toast(name+' загружена');
  }catch(e){
    console.error(e); live2dStatus('ERROR');
    aiEl('live2dModelInfo').textContent='Ошибка: '+e.message;
...
function setupBundledModelButtons(){
  aiEl('live2dMikuSample')?.addEventListener('click',()=>live2dLoadBundledFile(new URL('miku-sample-model3.json',document.baseURI).href,'Miku Sample'));
  aiEl('live2dMikuFree')?.addEventListener('click',()=>live2dLoadBundledFile(new URL('miku-free-model3.json',document.baseURI).href,'Miku Free'));
}
function setupLive2D(){
  setupBundledModelButtons();
