...
function aiAdd(role,text){const chat=aiEl('aiChat');if(!chat)return null;const box=document.createElement('div');box.className='ai-msg '+role;box.innerHTML='<b>'+(role==='user'?'YOU':'ALDUIN AI')+'</b><p></p>';box.querySelector('p').textContent=text;chat.appendChild(box);chat.scrollTop=chat.scrollHeight;return box.querySelector('p')}
...
function findLiveFile(ref){
  const n=fileKey(ref); if(live2dFiles.has(n))return live2dFiles.get(n);
  const base=n.split('/').pop();
  for(const [k,v] of live2dFiles)if(k.split('/').pop()===base)return v;
  return null;
}
function makeLiveURL(file){
  const blob=file?.blob||file; const u=URL.createObjectURL(blob); live2dObjectURLs.push(u); return u;
}
function rewriteLiveJSON(value){
  if(typeof value==='string'){
    const f=findLiveFile(value);
    if(f && /\.(moc3|png|jpg|jpeg|webp|json|motion3|exp3|physics3|pose3|userdata3|cdi3)$/i.test(value))return makeLiveURL(f);
    return value;
  }
  if(Array.isArray(value))return value.map(rewriteLiveJSON);
  if(value&&typeof value==='object'){const out={};for(const [k,v] of Object.entries(value))out[k]=rewriteLiveJSON(v);return out}
  return value;
}
...
async function readLiveText(file){
  ...
  throw new Error('Этот браузер не умеет читать файл модели как текст.');
}
async function readModelFromFiles(files){
  ...
  for(const f of files){
    ...
    const entry=liveFileEntry(path,f,f.name);
    entries.push(entry); live2dFiles.set(fileKey(path),entry);
  }
  const modelFile=chooseModelFile(entries);
  if(!modelFile)throw new Error('Не найден .model3.json. Выбери ZIP или всю папку Live2D-модели.');
  const raw=await readLiveText(modelFile.blob);
  const json=JSON.parse(raw); const rewritten=rewriteLiveJSON(json);
...
async function loadLive2D(data){
  if(!window.PIXI?.live2d?.Live2DModel)throw new Error('Live2D runtime не загрузился. Проверь интернет.');
...
async function live2dLoadBundled(){try{live2dStatus('LOADING…');if(!window.PIXI?.live2d?.Live2DModel)throw new Error('Live2D runtime не загрузился.');if(live2dApp){try{live2dApp.destroy(true,{children:true,texture:true,baseTexture:true})}catch{}live2dApp=null;live2dModel=null}const canvas=aiEl('live2dCanvas'),host=aiEl('live2dStage');live2dApp=new PIXI.Application({view:canvas,autoStart:true,resizeTo:host,transparent:true,antialias:true,resolution:Math.min(2,window.devicePixelRatio||1)});live2dModel=await PIXI.live2d.Live2DModel.from('models/miara/model3.json',{autoInteract:false,autoUpdate:true});live2dApp.stage.addChild(live2dModel);live2dCenter();live2dModel.on?.('hit',()=>{if(aiEl('live2dAI')?.checked){live2dEmotion('happy');live2dMotion('tap')}});aiEl('live2dEmpty').hidden=true;live2dStatus('MODEL ONLINE',true);aiEl('live2dModelInfo').textContent='✓ Встроенная Miara • Cubism 3 • 7 файлов';live2dMotion('auto');save(LIVE2D_KEY,{name:'Встроенная Miara'});toast('Встроенная Live2D загружена')}catch(e){console.error(e);live2dStatus('ERROR');aiEl('live2dModelInfo').textContent='Ошибка встроенной модели: '+e.message;toast('Live2D: '+e.message)}}
