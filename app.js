...
async function ensureWebLLM(){
  ...
  try{
    ...
    if(note)note.textContent="Не удалось загрузить Local AI. Для первого запуска нужен интернет.";
    return null;
  }
}
function checkLocalAI(){
  const load=aiEl("aiLoad"), note=aiEl("aiCapability");
  if(!navigator.gpu){
  ...
  if(note)note.textContent="WebGPU обнаружен. Модель будет загружена только после нажатия кнопки.";
  return true;
}
...
function setupALDUINAI(){
  ...
  // Проверяем только лёгкую поддержку WebGPU. WebLLM НЕ загружается при старте сайта.
  checkLocalAI();
  ...
  if(form)form.addEventListener("submit",async e=>{
    ...
    aiAdd("user",text);input.value="";await sendALDUINAI(text);
  });
}
