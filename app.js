...
async function boot(){
  const loader=$('loader');
  const hideLoader=()=>{ if(loader){ loader.classList.add('hide'); setTimeout(()=>loader.remove(),500); } };
  try{
    // Критический запуск интерфейса — не ждём IndexedDB, галерею или другие фоновые операции.
    ...
    // Галерея загружается после показа интерфейса.
    try{
      await openDB();
      photos=await dbAll();
      renderAll();
    }catch(e){ console.warn('Gallery DB unavailable',e); }
  }catch(e){
    console.error('ALDUIN startup',e);
    // Даже при ошибке второстепенной функции сайт не должен оставаться на loader.
    hideLoader();
  }
}
